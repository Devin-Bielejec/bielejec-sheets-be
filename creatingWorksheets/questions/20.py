...
class _20():
    def __init__(self, difficulty=1):
        ...
        #diff2 (dx^a+b)(ex^[not a]+c) where gcd(d,b)=1 and gcd(e,c)=1 nothing 0 b,c not -ps's
        ...
        #diff2 (ax^b+c)(dx^e[not a]+f) where gcd(a,c)=1 and gcd(d,f)=1 nothing 0 b,c not -ps's
        elif difficulty == 2:
          a = randomBetweenNot(-12,12,[0])
          b = random.choice([1,5])
          c = random.choice([x for x in range(-12,13) if (gcd(x,a)==1 or gcd(x,a)==-1) and x not in [-9,-4,-1,0]])
          d = randomBetweenNot(-12,12,[0])
          e = randomBetweenNot(1,5,[b])
          f = random.choice([x for x in range(-12,13) if (gcd(x,d)==1 or gcd(x,d)==-1) and x not in [-9,-4,-1,0]]) 
        #diff3 (ax^b=2+c)(dx^e[not 2]+f) where gcd(a,c)1,-1 but c -ps
        elif difficulty == 3:
          a = random.choice([x**2 for x in range(1,13)])
          aRoot = int(a**(1/2))
          b = random.choice([2])
          c = random.choice([-1*x**2 for x in range(1,13) if gcd(-1*x**2,a)==1 or gcd(-1*x**2,b)==-1])
          cRoot = int((c*-1)**(1/2))
          d = randomBetweenNot(-12,12,[0])
          e = randomBetweenNot(1,5,[b])
          f = random.choice([x for x in range(-12,13) if (gcd(x,d)==1 or gcd(x,d)==-1) and x not in [-9,-4,-1,0]])      
